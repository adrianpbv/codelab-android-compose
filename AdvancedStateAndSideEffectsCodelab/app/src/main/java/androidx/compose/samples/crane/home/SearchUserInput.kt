...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    // TODO 6 State holder caller, remember the state here
   ...
   // TODO 6.1 notify when the input changes
    val currentOnDestinationChanged by rememberUpdatedState(onToDestinationChanged)
    // trigger a side-effect using LaunchEffect
    LaunchedEffect(editableUserInputState) {
        // The snapshotFlow API converts Compose State<T> objects into a Flow
        snapshotFlow { editableUserInputState.text }
            .filter { !editableUserInputState.isHint }
            .collect {
                //collect the emitted items to notify the parent that the current destination changed.
